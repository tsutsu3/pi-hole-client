...
the update will automatically enable **both** **"Allow self-signed certificates"**
AND **"Don't check SSL certificate"** for backward compatibility.

This means all your HTTPS servers will show **"Don't check SSL certificate"**
status, and you'll see a warning banner:
...
all HTTPS servers will show **"Don't check SSL certificate"** status.
This is less secure than the pinned certificate approach.

If it was **OFF** before the update, no changes occur—your servers will remain
at **"HTTPS Verified"** or **"HTTPS Untrusted Blocked"** status.
:::
...
<img width="320" alt="Allow self-signed certificate confirmation modal" src={imgAllowSelfSignedModal} />

After confirming,
your server will show **"HTTPS Pinned"** status, which provides better security
than "Don't check SSL certificate".
