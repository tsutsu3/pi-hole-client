...
sidebar:
  label: Certificate Configuration Guide
  order: 4
---
...
<img width="320" alt="All certificate statuses" src={imgStatusAll.src} />
...
<img width="512" alt="HTTPS Verified status" src={imgStatusVerify.src} />
...
<img width="512" alt="HTTPS Pinned status" src={imgStatusPinned.src} />
...
<img width="320" alt="Server menu with Update option" src={imgServerMenu.src} />
:::
...
<img width="512" alt="Don't check SSL certificate status" src={imgStatusSkip.src} />
...
<img width="512" alt="HTTPS Untrusted Allowed status" src={imgStatusUntrustedAllow.src} />
...
<img width="512" alt="HTTP status" src={imgStatusHttp.src} />
...
<img width="512" alt="HTTPS Pin Mismatch status" src={imgStatusMismatch.src} />
...
<img width="512" alt="HTTPS Untrusted Blocked status" src={imgStatusBlock.src} />
...
<img width="320" alt="Self-signed certificate setting" src={imgSelfSignedCert.src} />

:::note
If your Pi-hole server uses a **valid CA-signed certificate** (e.g., Let's Encrypt), you should turn OFF "Allow untrusted certificates" to achieve the "HTTPS Verified" status.
:::

<img width="320" alt="Signed certificate setting" src={imgSignedCert.src} />

---

## After App Update: Backward Compatibility

:::note[v1.8.0 Update Notice]
...
**"Allow untrusted certificates"** enabled before the update are
...
If you had **"Allow untrusted certificates"** enabled before updating the app,
the update will automatically enable **both**
**"Allow untrusted certificates"** **and**
...
<img width="320" alt="Warning banner after app update" src={imgWarningBar.src} />

:::caution
After the app update,
if **"Allow untrusted certificates"** was **ON** before the update,
...
If it was **OFF** before the update, no changes are applied—
...
<ImageRow>
  <img width="240" alt="Self-signed certificate settings" src={imgSelfSignedCert.src} />
  <img
    width="240"
    alt="Allow self-signed certificate confirmation modal"
    src={imgAllowSelfSignedModal.src}
  />
</ImageRow>
...
<img width="320" alt="Docker connection error" src={imgDockerError.src} />
...
<img width="320" alt="Ignore certificate errors setting" src={imgIgnoreCertError.src} />
...
If you're using a valid Let's Encrypt certificate but still get SSL errors in the app,
the most common cause is an **incomplete certificate chain**.

Unlike web browsers, which can automatically fetch missing intermediate certificates,
Flutter's HTTP client requires the server to provide the **complete chain**.
...
:::caution
This is a temporary workaround. Fixing the certificate chain on the server side is strongly recommended for proper security.
:::
...
- [Q2. SSL error with a valid Let's Encrypt certificate](../../help/faq/#-q2-ssl-error-with-a-valid-lets-encrypt-certificate)
- [Q3. "Connection failed" with self-signed certificate](../../help/faq/#-q3-connection-failed-with-self-signed-certificate)
- [Q4. Certificate pin mismatch after server certificate update](../../help/faq/#-q4-certificate-pin-mismatch-after-server-certificate-update)
- [Q5. Warning banner about unverified certificates keeps appearing](../../help/faq/#-q5-warning-banner-about-unverified-certificates-keeps-appearing)
