...
Future upgradeDbToV8(Database db) async {
  await db
      .execute("ALTER TABLE appConfig ADD COLUMN reducedDataCharts NUMERIC");
...
Future upgradeDbToV12(Database db) async {
  await db
      .execute("ALTER TABLE appConfig ADD COLUMN importantInfoReaden NUMERIC");
...
Future upgradeDbToV14(Database db) async {
  await db.execute(
      "ALTER TABLE appConfig ADD COLUMN statisticsVisualizationMode NUMERIC");
...
Future upgradeDbToV15(Database db) async {
  ...
  await db.execute(
      "CREATE TABLE servers (address TEXT PRIMARY KEY, alias TEXT, token TEXT, isDefaultServer NUMERIC)");

  List<Future> futures = [];
  for (var server in servers) {
    futures.add(db.execute(
        "INSERT INTO servers (address, alias, token, isDefaultServer) VALUES ('${server.address}', '${server.alias}', '${server.token}', ${server.defaultServer == true ? 1 : 0})"));
  }
